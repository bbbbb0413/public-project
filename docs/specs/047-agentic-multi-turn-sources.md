...
- `public-front/src/components/AiService.tsx:693` (`handleSendQuestion`) 및 `public-front/src/api/ai.ts:488-490` (`askQuestionStream`)은 SSE의 `sources` 이벤트를 수신하여 `setCurrentSources`로 상태를 갱신하도록 이미 준비되어 있으나, 백엔드에서 2회차 이후 갱신된 출처 이벤트를 발행하지 않아 최종 답변에 사용된 전체 근거 문서 목록을 화면에서 확인할 수 없다.
...
- [ ] `public-python-server/src/ai_service/rag/application/agentic_ask_use_case.py`에서 매 반복(`iteration`)마다 새롭게 검색된 청크들을 누적(또는 병합)하여 중복 없이 전체 출처 목록(`SourceRef`)을 생성하고, 청크가 추가될 때마다 최신 출처 목록을 `__SOURCES:{json}` 형태로 방출한다.
...
- `public-python-server/src/ai_service/rag/application/agentic_ask_use_case.py`:
  - 루프 진입 전 누적 청크/출처 컬렉션(예: `accumulated_sources: dict[str, dict[str, Any]]`)을 초기화.
  - 매 반복에서 `chunks`를 순회하며 청크 고유 키(`${documentId}:${chunkIndex}`)를 기반으로 중복 제거 및 누적.
  - 해당 반복에서 새로운 청크가 추가되거나 출처 목록이 갱신된 경우, 전체 누적 출처 목록을 직렬화하여 `yield f"__SOURCES:{json.dumps(sources)}"` 방출.
