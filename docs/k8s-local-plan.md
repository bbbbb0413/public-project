...
작성 2026-08-29 · 개정 2026-08-30. 진행하며 갱신한다.
...
| 트랙 | 내용 | 성격 |
|---|---|---|
| **A. 로컬 구동** | 클러스터·CI → ArgoCD → 상태 저장소 → 앱 → 시크릿 → 내부 검증 | 필수 경로 |
...
| Istio·Vault | 로컬에서 제외 | 앱을 띄우기까지의 거리를 줄인다. 나중에 하나씩 붙인다 |
...
minikube start -p public-project \
  --driver docker \
  --cpus 4 --memory 10g --disk-size 40g \
...
| 쿠버네티스 시스템 | — | 약 1Gi | kubelet·CoreDNS·ingress·metrics-server |
| **합계** | **약 1.7 vCPU** | **약 7.9Gi** | |

10GB에 2.1GB가 남는다. `limits` 는 메모리에만 걸고 CPU에는 걸지 않는다. CPU
상한은 스로틀링을 만들어 로컬에서 원인을 찾기 어려운 지연을 만든다.
...
1. `metrics-server` 애드온을 끈다 (HPA를 로컬에서 쓰지 않으면 불필요하다)
2. frontend를 클러스터에서 빼고 `npm run dev` 로 호스트에서 돌린다
3. 지금 작업과 무관한 앱의 `replicas` 를 0으로 둔다

Harbor의 trivy를 켤 때는 3번으로 자리를 만든다.
...
compose의 환경변수를 ConfigMap으로 옮기며 호스트명이 전부 바뀐다.
...
**public-infra는 공개 저장소라 평문으로 커밋할 수 없다.**

`.env` 에서 Secret을 만드는 부트스트랩 단계로 처리하고, 앱 차트는
`vault.enabled: false` 로 둔다. GitOps에서도 최초 시크릿 하나는 클러스터 밖에서
넣어야 하므로 day-0 수동 단계로 명시한다.

```bash
kubectl create secret generic app-secrets -n local-app --from-env-file=.env
```

**완료 기준**: 파드가 API 키를 환경변수로 받고, `.env` 값이 git에 남지 않는다.

## A5. 내부 진입점과 검증

ingress 애드온으로 `gateway` 와 `frontend` 를 노출한다. 아직 외부 공개는 아니다.

- `/etc/hosts` 에 `127.0.0.1 public.local argocd.local` 매핑
...
- `ACCESS_TOKEN_SECRET`·`REFRESH_TOKEN_SECRET`·`CHALLENGE_PRIVATE_KEY` 를
  `dev.values.yaml`·`staging.values.yaml` 에서 제거하고 Secret으로 옮긴다.
  **값도 새로 만든다** — 공개 저장소에 노출된 값이다
...
- **복제 규칙**을 만든다. `ghcr.io` 를 원본으로 하는 pull 기반 규칙을 5분 주기로
  건다. pull 기반 복제는 이벤트로 즉시 트리거되지 않고 스케줄 또는 수동이다
- **로봇 계정**으로 클러스터에 pull 전용 권한만 준다
- 보존 정책(태그 N개 유지), 가비지 컬렉션 주기, 프로젝트 쿼터

**부팅 순서** — Harbor 자신의 이미지는 Harbor를 통과할 수 없다. Harbor·ArgoCD·ingress
이미지는 업스트림에서 직접 받는다. 앱 이미지에 `imagePullPolicy: IfNotPresent` 를
두어 Harbor가 죽어도 이미 받아 둔 이미지로 재기동되게 한다.
...
- Istio 도입과 gateway 노출 경로 전환
- Vault 연동으로 시크릿 주입 방식 통일
